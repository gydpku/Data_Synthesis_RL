...
from eval.tasks.cqa.process_prediction import process_prediction
from eval.tasks.cqa.get_output_instruction import get_output_instruction
import os
...
def process_and_save_dataset(train_data, data_source,local_dir):
    ...
    ori_test_dataset = load_dataset("nguha/legalbench", name="consumer_contracts_qa", split="test")
    #ori_test_dataset = load_dataset('csv', data_files="test.csv")
    #ori_train_dataset = load_dataset("nguha/legalbench", name="consumer_contracts_qa", split="train")
    test_data=[]
    for doc in ori_test_dataset:
        query = f"""Contract: {doc['contract']}
