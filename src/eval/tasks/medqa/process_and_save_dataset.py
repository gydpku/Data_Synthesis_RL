...
def process_and_save_dataset(train_data, data_source,local_dir):
    ...
    '''
    ori_train_dataset = datasets.load_dataset('GBaker/MedQA-USMLE-4-options', split='train', trust_remote_code=True)
    train_data=[]
    for doc in ori_train_dataset:
        query = doc['question']+ ' ' #preprocess(doc["activity_label"] + ": " + ctx)
        options=[doc['options'][key] for key in doc['options']]
        choices = [str(chr(id+ord('A')))+': '+option for id,option in enumerate(options)] #[preprocess(ending) for ending in doc["endings"]]
        query+=' '.join(choices)
        gold = doc['answer_idx']
    ...
    ori_test_dataset = datasets.load_dataset('GBaker/MedQA-USMLE-4-options', split='test', trust_remote_code=True)
    test_data=[]
